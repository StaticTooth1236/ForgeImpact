...
// Replay pacing: recorded gaps are divided by SPEED and capped at MAX_GAP_MS,
// so a ~10-minute recorded run replays in roughly 1.5–2 minutes while keeping
// the live rhythm (bursts, pauses, streaming).
const REPLAY_SPEED = 6;
const REPLAY_MAX_GAP_MS = 1200;

// Preset texts must stay EXACTLY in sync with record_demo.py — replay
// availability is decided by exact text match.
const PRESETS = [
  {
...
export default function App() {
  ...
  // Replay a recorded run with live-like pacing. Returns false if no
  // recording exists for this exact text.
  ...
  async function runAnalysis() {
    ...
    // Unmodified preset with a recording → free replay with live feel.
    // Anything else → live run through the API (access code enforced server-side).
    ...
    <div className="page">
      ...
      <header className="titleblock">
        <div className="tb-cell tb-brand">
          <span className="tb-name">ARIA</span>
          <span className="tb-sub">Agentic Reasoning for Impact Analysis</span>
        </div>
        <div className="tb-cell">
          <span className="tb-key">Program</span>
          <span className="tb-val">Eurus Systems · MAAP-1</span>
        </div>
        <div className="tb-cell">
      ...
      <section className="intake">
        ...
        <p className="fineprint">
          ...
          preset scenarios replay recorded runs · custom changes run live and
          need an access code
        </p>
        {errorMsg && <div className="errorbar">Analysis halted: {errorMsg}</div>}
      </section>

      {/* ---- Mission board + PMP sheet -------------------------------- */}
      {showBoard && (
